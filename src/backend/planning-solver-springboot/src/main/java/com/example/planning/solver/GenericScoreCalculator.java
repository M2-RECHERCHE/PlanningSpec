...
public class GenericScoreCalculator implements EasyScoreCalculator<GenericPlanningProblem, HardSoftScore> {
    ...
    public HardSoftScore calculateScore(GenericPlanningProblem solution) {
        ...
        hardResourceDayRules(solution, score, support);
        hardPrecedence(solution, score, support);

        softAvoidParticipation(solution, score, support);
        softResourceDayPreferences(solution, score, support);
    ...
    private void hardResourceDayRules(GenericPlanningProblem solution, ScoreAccumulator score, ScoreSupport support) {
        for (ResourceRequiredDayRule rule : solution.getResourceRequiredDayRuleList()) {
            int outsideTargetDay = support.activitiesUsingResource(rule.getResourceId()).stream()
                    .map(ActivityInstance::getDay)
                    .filter(day -> day != null && !rule.getDay().equals(day))
                    .toList()
                    .size();
            score.hard("resource_required_day", outsideTargetDay);
        }

        for (ResourceSingleDayRule rule : solution.getResourceSingleDayRuleList()) {
            int distinctDays = support.resourceUsedDayCount(rule.getResourceId());
            if (distinctDays > 1) {
                score.hard("resource_single_day", distinctDays - 1);
            }
        }
    }
    ...
    private void softResourceDayPreferences(GenericPlanningProblem solution, ScoreAccumulator score, ScoreSupport support) {
        for (PreferResourceOnDatePreference preference : solution.getPreferResourceOnDatePreferenceList()) {
            int outsidePreferredDay = support.activitiesUsingResource(preference.getResourceId()).stream()
                    .map(ActivityInstance::getDay)
                    .filter(day -> day != null && !preference.getDay().equals(day))
                    .toList()
                    .size();
            score.soft("prefer_resource_on_date", outsidePreferredDay * preference.getWeight());
        }

        for (PreferResourceSingleDayPreference preference : solution.getPreferResourceSingleDayPreferenceList()) {
            int distinctDays = support.resourceUsedDayCount(preference.getResourceId());
            if (distinctDays > 1) {
                score.soft("prefer_resource_single_day", (distinctDays - 1) * preference.getWeight());
            }
        }
    }
    ...
    static final class ScoreSupport {
        ...
        List<ActivityInstance> activitiesUsingResource(String resourceId) {
            return problem.getActivityInstanceList().stream()
                    .filter(activity -> activityUsesResource(activity.getId(), resourceId))
                    .toList();
        }

        int resourceUsedDayCount(String resourceId) {
            return (int) activitiesUsingResource(resourceId).stream()
                    .map(ActivityInstance::getDay)
