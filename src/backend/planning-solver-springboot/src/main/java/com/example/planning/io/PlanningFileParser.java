...
public class PlanningFileParser {
    ...
    private void parseConstraints(JsonNode root, GenericPlanningProblem problem, Map<String, Resource> resourcesById,
        ...
        for (JsonNode c : constraints) {
            ...
            switch (type) {
                ...
                case "resource_required_day" -> problem.getResourceRequiredDayRuleList().add(new ResourceRequiredDayRule(
                        requiredText(c, "resource"), requiredText(c, "date")));
                case "resource_single_day" -> problem.getResourceSingleDayRuleList().add(new ResourceSingleDayRule(
                        requiredText(c, "resource")));
    ...
    private void parsePreferences(JsonNode root, GenericPlanningProblem problem, Map<String, Resource> resourcesById,
        ...
        for (JsonNode p : preferences) {
            ...
            switch (type) {
                ...
                case "prefer_resource_on_date" -> problem.getPreferResourceOnDatePreferenceList().add(new PreferResourceOnDatePreference(
                        requiredText(p, "resource"), requiredText(p, "date"), required(p, "weight").asInt(0)));
                case "prefer_resource_single_day" -> problem.getPreferResourceSingleDayPreferenceList().add(new PreferResourceSingleDayPreference(
                        requiredText(p, "resource"), required(p, "weight").asInt(0)));
    ...
    private void validateReferences(GenericPlanningProblem problem, Map<String, Resource> resourcesById,
        ...
        for (ResourceRequiredDayRule r : problem.getResourceRequiredDayRuleList()) {
            requireResource(resourcesById, r.getResourceId(), "resource_required_day.resource");
            if (!problem.getDayList().contains(r.getDay())) throw new PlanningRequestException("resource_required_day.date inconnue: " + r.getDay());
        }
        for (ResourceSingleDayRule r : problem.getResourceSingleDayRuleList()) {
            requireResource(resourcesById, r.getResourceId(), "resource_single_day.resource");
        }
        ...
        for (PreferResourceOnDatePreference p : problem.getPreferResourceOnDatePreferenceList()) {
            requireResource(resourcesById, p.getResourceId(), "prefer_resource_on_date.resource");
            if (!problem.getDayList().contains(p.getDay())) throw new PlanningRequestException("prefer_resource_on_date.date inconnue: " + p.getDay());
        }
        for (PreferResourceSingleDayPreference p : problem.getPreferResourceSingleDayPreferenceList()) {
            requireResource(resourcesById, p.getResourceId(), "prefer_resource_single_day.resource");
        }
