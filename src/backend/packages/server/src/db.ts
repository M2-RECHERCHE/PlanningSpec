...
export async function updatePlanningSolutionVersionStatus(
    id: string,
    planningId: string,
    userId: string,
    status: PlanningSolutionStatus,
    ...
    const [result] = await database.execute<ResultSetHeader>(`
        ...
        WHERE id = ?
          AND planning_id = ?
          AND user_id = ?
    `, [status, solutionKind, id, planningId, userId]);

    if (result.affectedRows === 0) {
        return null;
    }

    return getPlanningSolutionVersionById(id, planningId, userId);
}
