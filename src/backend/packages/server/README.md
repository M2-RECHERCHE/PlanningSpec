...
Pour MiniZinc et OptaPlanner, aucune limite de temps n'est appliquée par défaut. Vous pouvez fournir
`solverTimeLimitSeconds` dans le body pour passer explicitement une limite à la résolution.
...
- `OPTAPLANNER_TIMEOUT_MS` : timeout HTTP de l'appel OptaPlanner synchrone. `0` par défaut signifie aucune coupure automatique et ne limite pas le solveur.
...
Les valeurs sont chargées depuis `src/backend/packages/server/.env` au démarrage, puis peuvent être surchargées par les variables d'environnement du processus.
