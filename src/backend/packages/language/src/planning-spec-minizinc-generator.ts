...
export class PlanningSpecMiniZincGenerator {
  ...
  generateToMiniZinc(document: LangiumDocument): string {
    ...
    const dayIndexOf = (dateStr: string): number => days.findIndex(d => d === dateStr) + 1;
    const resourceUsedOnDayExpr = (res: string, dayExpr: string): string =>
      `sum(a in ACT_INST) (bool2int(assignment[a, ${res}] /\\ dayOf(start_time[a]) == ${dayExpr})) > 0`;
    const resourceUsedDayCountExpr = (res: string): string =>
      `sum(d in 1..NUM_DAYS) (bool2int(${resourceUsedOnDayExpr(res, 'd')}))`;
    ...
    model.constraints.constraints.forEach((c: any) => {
      ...
      // ResourceRequiredDay
      // ---------------------------
      if (c.$type === 'ResourceRequiredDay') {
        const res = this.id(c.resource);
        const dateStr = this.s(c.date);
        const dayIndex = dayIndexOf(dateStr);

        code += `% ResourceRequiredDay: ${res} only on ${dateStr}\n`;
        if (dayIndex <= 0) {
          code += `% ERROR: date "${dateStr}" not found in days list\n`;
          code += `constraint false;\n\n`;
        } else {
          code += `constraint forall(a in ACT_INST) (\n`;
          code += `  assignment[a, ${res}] -> dayOf(start_time[a]) == ${dayIndex}\n`;
          code += `);\n\n`;
        }
      }
      ...
      // ResourceSingleDay
      // ---------------------------
      if (c.$type === 'ResourceSingleDay') {
        const res = this.id(c.resource);

        code += `% ResourceSingleDay: ${res} must be grouped on at most one day\n`;
        code += `constraint ${resourceUsedDayCountExpr(res)} <= 1;\n\n`;
      }
    });
    ...
    model.preferences.preferences.forEach((p: any) => {
      ...
      if (p.$type === 'AvoidParticipationOnDate') {
        ...
        const dayIndex = dayIndexOf(dateStr); // 1-based
        if (dayIndex <= 0) {
      ...
      // PreferResourceOnDate
      if (p.$type === 'PreferResourceOnDate') {
        const res = this.id(p.resource);
        const dateStr = this.s(p.date);
        const weight = p.weight;
        ...
        if (dayIndex <= 0) {
          ...
          penaltyTerms.push(
            `sum(a in ACT_INST) ( ${weight} * bool2int(assignment[a, ${res}] /\\ dayOf(start_time[a]) != ${dayIndex}) )`
          );
        }
      }

      // PreferResourceSingleDay
      if (p.$type === 'PreferResourceSingleDay') {
        const res = this.id(p.resource);
        const weight = p.weight;

        penaltyTerms.push(
          `${weight} * max(0, ${resourceUsedDayCountExpr(res)} - 1)`
        );
      }
