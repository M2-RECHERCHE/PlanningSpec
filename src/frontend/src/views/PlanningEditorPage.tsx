...
const CONSTRAINT_TYPES = [
  ...
  { value: 'resource_required_day', label: 'Jour imposé ressource' },
  { value: 'resource_single_day', label: 'Une seule journée ressource' },
] as const;
...
const PREFERENCE_TYPES = [
  ...
  { value: 'prefer_resource_on_date', label: 'Préférer une date' },
  { value: 'prefer_resource_single_day', label: 'Préférer une seule journée' },
...
const isConstraintType = (value: unknown): value is ConstraintType =>
  ...
  value === 'resource_required_day' ||
  value === 'resource_single_day';
...
const normalizeConstraint = (constraint: Record<string, any>): EditorConstraint => ({
  ...
  date: typeof constraint.date === 'string' ? constraint.date : DAYS[0],
...
const isPreferenceType = (value: unknown): value is PreferenceType =>
  ...
  value === 'prefer_resource_on_date' ||
  value === 'prefer_resource_single_day' ||
  value === 'max_per_scope' ||
...
const describeConstraint = (constraint: EditorConstraint) => {
  ...
  if (constraint.type === 'resource_required_day') {
    return `${constraint.resource || 'Ressource'} uniquement le ${constraint.date || 'jour choisi'}`;
  }
  if (constraint.type === 'resource_single_day') {
    return `${constraint.resource || 'Ressource'} regroupée sur une seule journée`;
  }
...
const buildSolvePayload = (formData: EditorFormData) => {
  ...
  const preferences = formData.preferences
    ...
    .map(preference => {
      ...
      if (preference.type === 'prefer_resource_on_date') {
        return {
          type: preference.type,
          resource: preference.resource?.trim() || '',
          date: preference.date?.trim() || '',
      ...
      if (preference.type === 'prefer_resource_single_day') {
        return {
          type: preference.type,
          resource: preference.resource?.trim() || '',
          weight: Math.max(1, Number(preference.weight) || 1),
...
const buildPlanningSchema = (formData: EditorFormData) => {
  ...
  const dayOptions = formData.days.length > 0 ? formData.days : DAYS;
    ...
    properties: {
      ...
      constraints: {
        ...
        defaultSnippets: [
          ...
          {
            label: 'Jour imposé ressource',
            body: [{
              type: 'resource_required_day',
              resource: resourceNames[0] ?? 'Teacher_1',
              date: dayOptions[0] ?? 'Lundi',
            }],
          },
          {
            label: 'Une seule journée ressource',
            body: [{
              type: 'resource_single_day',
              resource: resourceNames[0] ?? 'Teacher_1',
            }],
          },
        ],
        items: {
          oneOf: [
            ...
            {
              type: 'object',
              required: ['type', 'resource', 'date'],
              additionalProperties: false,
              properties: {
                type: { const: 'resource_required_day' },
                resource: { type: 'string', enum: resourceNames.length > 0 ? resourceNames : undefined },
                date: { type: 'string', enum: dayOptions },
              },
            },
            {
              type: 'object',
              ...
              additionalProperties: false,
              properties: {
                type: { const: 'resource_single_day' },
                resource: { type: 'string', enum: resourceNames.length > 0 ? resourceNames : undefined },
              },
            },
          ],
      ...
      preferences: {
        ...
        defaultSnippets: [
          ...
          {
            label: 'Préférer une date',
            body: [{
              type: 'prefer_resource_on_date',
              resource: resourceNames[0] ?? 'Teacher_1',
              date: dayOptions[0] ?? 'Lundi',
              weight: 10,
            }],
          ...
          {
            label: 'Préférer une seule journée',
            body: [{
              type: 'prefer_resource_single_day',
              resource: resourceNames[0] ?? 'Teacher_1',
              weight: 10,
            }],
          },
        ...
        items: {
          oneOf: [
            ...
            },
            {
              ...
              additionalProperties: false,
              properties: {
                type: { const: 'prefer_resource_on_date' },
                resource: { type: 'string', enum: resourceNames.length > 0 ? resourceNames : undefined },
                date: { type: 'string', enum: dayOptions },
                weight: { type: 'integer', minimum: 1 },
              },
            ...
            {
              type: 'object',
              required: ['type', 'resource', 'weight'],
              additionalProperties: false,
              properties: {
                type: { const: 'prefer_resource_single_day' },
                resource: { type: 'string', enum: resourceNames.length > 0 ? resourceNames : undefined },
                weight: { type: 'integer', minimum: 1 },
              },
            },
...
const getStepError = (step: number, data: EditorFormData): string | null => {
  ...
  if (step === 6) {
    ...
    const invalidConstraint = data.constraints.find(constraint => {
      ...
      if (constraint.type === 'resource_required_day') {
        return !constraint.resource?.trim() ||
          !resourceInstances.has(constraint.resource.trim()) ||
          !constraint.date?.trim() ||
          !data.days.includes(constraint.date.trim());
      }
      ...
      return (
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '12px', alignItems: 'end' }}>
          <Select label="Ressource" value={constraint.resource || ''} onChange={value => updateConstraint(constraint.id, { resource: value })}
            options={[{ value: '', label: '— Ressource —' }, ...resourceInstanceOptions]}
          />
          <Select label="Jour imposé" value={constraint.date || ''} onChange={value => updateConstraint(constraint.id, { date: value })}
...
const Step7Preferences: React.FC<{ data: EditorFormData; onChange: (d: EditorFormData) => void }> = ({ data, onChange }) => {
  ...
  const renderPreferenceFields = (preference: EditorPreference) => {
    if (preference.type === 'avoid_participation_on_date' || preference.type === 'prefer_resource_on_date') {
      return (
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : isCompact ? '1fr 1fr' : '1fr 1fr 0.8fr', gap: '12px', alignItems: 'end' }}>
          <Select label="Ressource" value={preference.resource || ''} onChange={value => updatePreference(preference.id, { resource: value })}
            options={[{ value: '', label: '— Ressource —' }, ...resourceInstanceOptions]}
          />
          <Select label={preference.type === 'avoid_participation_on_date' ? 'Jour à éviter' : 'Jour préféré'} value={preference.date || ''} onChange={value => updatePreference(preference.id, { date: value })}
            options={(data.days.length > 0 ? data.days : DAYS).map(day => ({ value: day, label: day }))}
          />
    ...
    if (preference.type === 'prefer_resource_single_day') {
      return (
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 0.8fr', gap: '12px', alignItems: 'end' }}>
          <Select label="Ressource" value={preference.resource || ''} onChange={value => updatePreference(preference.id, { resource: value })}
            options={[{ value: '', label: '— Ressource —' }, ...resourceInstanceOptions]}
          />
          <NumberInput label="Poids" value={preference.weight ?? 10} onChange={value => updatePreference(preference.id, { weight: value })} min={1} max={999} />
        ...
        <div key={preference.id} style={{
          ...
          <div style={{ fontSize: '11px', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {preference.type === 'avoid_participation_on_date'
              ...
              : preference.type === 'prefer_resource_on_date'
                ? `${preference.resource || '—'} préfère le ${preference.date || '—'} (poids : ${preference.weight ?? 10})`
                : preference.type === 'prefer_resource_single_day'
                  ? `${preference.resource || '—'} préfère une seule journée (poids : ${preference.weight ?? 10})`
...
function formatOptaPreviewMetrics(solution: OptaSolutionPreview): string {
  const penalty = typeof solution.objectiveValue === 'number'
    ? solution.objectiveValue
    : solution.penalty;
  const parts: string[] = [];
  if (typeof penalty === 'number') {
    parts.push(`pénalité=${penalty}`);
  }
  if (typeof solution.hardPenalty === 'number') {
    parts.push(`hardPenalty=${solution.hardPenalty}`);
  }
  if (typeof solution.softPenalty === 'number') {
    parts.push(`softPenalty=${solution.softPenalty}`);
  }
  if (typeof solution.constraintsSatisfied === 'boolean') {
    parts.push(`contraintes=${solution.constraintsSatisfied ? 'OK' : 'violées'}`);
  }
  if (typeof solution.preferencesSatisfied === 'boolean') {
    parts.push(`préférences=${solution.preferencesSatisfied ? 'OK' : 'non satisfaites'}`);
  }
  return parts.length > 0 ? ` · ${parts.join(' · ')}` : '';
}

function buildFeasibleSolutionsConsoleOutput(
  solutions: OptaSolutionPreview[],
  finalOutput?: string
): string {
  const history = solutions
    .map((solution, index) => `=== Solution faisable #${index + 1}${formatOptaPreviewMetrics(solution)} ===\n${solution.output}`)
    .join('\n\n');
...
export const PlanningEditorPage: React.FC = () => {
  ...
  const [solverTimeLimitSeconds, setSolverTimeLimitSeconds] = useState<string>('');
  ...
  const [optaBestPenalty, setOptaBestPenalty] = useState<number | null>(null);
  ...
  const pollOptaPlannerAsyncStatus = useCallback(async (planningId: string, jobId: string) => {
    try {
      const response = await api.get<{ data: {
        ...
        liveResult: (OptaSolutionPreview & { warnings: string[]; solveTimeMs: number; feasible: boolean; hardScore: number | null }) | null;
        liveSolutions: Array<OptaSolutionPreview & { warnings: string[]; solveTimeMs: number; feasible: boolean; hardScore: number | null }>;
        savedSolutions?: PlanningSolutionVersion[];
        finalized: boolean;
      ...
      const savedSolutions = Array.isArray(payload.savedSolutions) ? payload.savedSolutions : [];
      savedSolutions.forEach(mergeMiniZincSolution);
      ...
      if (typeof payload.bestPenalty === 'number') {
        setOptaBestPenalty(payload.bestPenalty);
      }
      if (liveSolutions.length > 0) {
        setSolverOutput(buildFeasibleSolutionsConsoleOutput(liveSolutions));
        const latest = liveSolutions[liveSolutions.length - 1];
        const latestPenalty = typeof latest.objectiveValue === 'number' ? latest.objectiveValue : latest.penalty;
        if (typeof latestPenalty === 'number') {
          setOptaBestPenalty(latestPenalty);
        }
        ...
        const liveMetrics = formatOptaPreviewMetrics(liveResult);
        const feasibilityLabel = liveResult.feasible
          ? `Dernière solution courante (faisable${liveMetrics})`
          : `Dernière solution courante (non faisable, hardScore=${liveResult.hardScore ?? 'n/a'}${liveMetrics})`;
        ...
        const livePenalty = typeof liveResult.objectiveValue === 'number' ? liveResult.objectiveValue : liveResult.penalty;
        if (typeof livePenalty === 'number') {
          setOptaBestPenalty(livePenalty);
        }
  ...
  }, [clearOptaPollTimer, loadPlanningVersions, mergeMiniZincSolution, optaStopRequested, refreshData, toast]);

  const startOptaPlannerAsyncSolve = useCallback(async (planningId: string, source: string) => {
    const parsedTimeLimit = Number(solverTimeLimitSeconds);
    const body: Record<string, unknown> = {
      solver: 'OptaPlanner',
      source,
    };
    if (solverTimeLimitSeconds.trim().length > 0 && Number.isFinite(parsedTimeLimit) && parsedTimeLimit > 0) {
      body.solverTimeLimitSeconds = Math.floor(parsedTimeLimit);
    }
              ...
              {selectedSolver === 'OptaPlanner' && (
                <div>
                  <div style={{ fontSize: 11, color: '#666', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                    Limite OptaPlanner (secondes, optionnelle)
                  </div>
                  <input
                    type="number"
                    min={1}
                    ...
                    onChange={(e) => {
                      ...
                      setSolverTimeLimitSeconds(String(Math.floor(next)));
                    }}
                    style={{
                      width: '100%',
                  ...
                  <div style={{ fontSize: 10, color: '#666', marginTop: 4 }}>
                    Vide = aucune limite automatique. Utilisez Stop OptaPlanner pour arrêter la résolution.
                  </div>
                  {solving && optaAsyncJobId && optaBestSolutionCount > 0 && (
                    <div style={{ fontSize: 10, color: '#93c5fd', marginTop: 4 }}>
                      ...
                      {optaBestPenalty !== null ? ` · pénalité courante: ${optaBestPenalty}` : ''}
                    </div>
                  )}
                </div>
              )}
