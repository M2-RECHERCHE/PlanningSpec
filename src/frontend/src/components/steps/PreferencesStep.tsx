...
const PREFERENCE_DESCRIPTIONS: Record<string, string> = {
  ...
  prefer_resource_on_date:
    'Favorise l\'affectation d\'une ressource sur un jour précis. Les participations hors de ce jour sont pénalisées selon le poids.',
  prefer_resource_single_day:
    'Favorise le regroupement de toutes les participations d\'une ressource sur une seule journée, sans imposer le jour.',
  max_per_scope:
...
const PreferencesStep: React.FC<Props> = ({ data, onUpdate }) => {
  ...
  const validateAndAdd = () => {
    ...
    if (prefType === 'avoid_participation_on_date' || prefType === 'prefer_resource_on_date') {
      if (!resource) errors.push('Ressource requise');
      if (!day) errors.push('Jour requis');
    } else if (prefType === 'prefer_resource_single_day') {
      if (!resource) errors.push('Ressource requise');
  ...
  const formatPrefLabel = (p: any): string => {
    ...
    if (p.type === 'prefer_resource_on_date') {
      return `${p.resource} préfère le ${p.date} (poids: ${p.weight})`;
    }
    if (p.type === 'prefer_resource_single_day') {
      return `${p.resource} préfère une seule journée (poids: ${p.weight})`;
    }
  ...
  const hasFormErrors = (): boolean => {
    ...
    if (prefType === 'prefer_resource_on_date') return !resource || !day;
    if (prefType === 'prefer_resource_single_day') return !resource;
    ...
    <div>
      ...
      <FormControl fullWidth sx={{ mb: 1 }}>
        ...
        >
          ...
          <MenuItem value="prefer_resource_on_date">Préférer un jour pour une ressource</MenuItem>
          <MenuItem value="prefer_resource_single_day">Préférer une seule journée</MenuItem>
      ...
      {(prefType === 'avoid_participation_on_date' || prefType === 'prefer_resource_on_date') && (
        <>
          ...
          <FormControl fullWidth sx={{ mb: 2 }}>
            <InputLabel>{prefType === 'avoid_participation_on_date' ? 'Jour à éviter *' : 'Jour préféré *'}</InputLabel>
            <Select value={day} onChange={e => setDay(e.target.value)} label={prefType === 'avoid_participation_on_date' ? 'Jour à éviter *' : 'Jour préféré *'}>
      ...
      {/* ── Préférer une seule journée ── */}
      {prefType === 'prefer_resource_single_day' && (
        <FormControl fullWidth sx={{ mb: 2 }}>
          <InputLabel>Ressource *</InputLabel>
          <Select value={resource} onChange={e => setResource(e.target.value)} label="Ressource *">
            <MenuItem value="" disabled>Choisir une ressource…</MenuItem>
