...
\newcommand{\codecell}[1]{\texttt{\footnotesize #1}}
...
\caption{Synthèse de la correspondance sémantique entre \DSL{} et \MZN{}.}
\label{tab:correspondance-dsl-minizinc}\\
\toprule
...
\codecell{time.days} &
\codecell{NUM\_DAYS}, fonction \codecell{dayOf} &
Les jours du DSL sont retrouvables à partir des créneaux globaux. \\

\codecell{slotsPerDay} &
\codecell{SLOTS\_PER\_DAY}, \codecell{TOTAL\_SLOTS} &
Le nombre total de créneaux est identique. \\

\codecell{activities} &
\codecell{ACT\_TYPE}, \codecell{ACT\_INST} &
Chaque activité et chaque instance sont conservées. \\

\codecell{duration} &
tableau \codecell{duration} &
La durée utilisée par le solveur est celle du DSL. \\

\codecell{resources} &
\codecell{RESOURCE}, \codecell{RES\_Type} &
Le typage des ressources est conservé. \\

\codecell{roles} &
\codecell{ROLE}, \codecell{role\_applicable}, \codecell{role\_assignment} &
Les rôles applicables et leurs types de ressources sont conservés. \\

\codecell{mandatory\_\allowbreak roles} &
somme des \codecell{role\_assignment} $\geq 1$ &
Chaque rôle obligatoire reçoit une ressource. \\

\codecell{cardinality\_\allowbreak per\_\allowbreak activity} &
sommes bornées par \codecell{min} et \codecell{max} &
Les nombres de ressources ou rôles affectés respectent les bornes. \\

\codecell{resource\_\allowbreak exclusivity} &
sommes par créneau ou par jour $\leq max$ &
Les limites d'utilisation des ressources sont conservées. \\

\codecell{fixed\_\allowbreak assignment} &
égalité booléenne à \codecell{true} &
L'affectation imposée est présente. \\

\codecell{forbidden\_\allowbreak assignment} &
égalité booléenne à \codecell{false} &
L'affectation interdite est absente. \\

\codecell{temporal\_\allowbreak precedence} &
\codecell{start\_before + duration <= start\_after} &
La précédence entre types d'activités est conservée. \\

\codecell{instance\_\allowbreak precedence} &
même inégalité entre instances &
La précédence entre instances est conservée. \\

\codecell{time\_\allowbreak window} &
bornes sur début et fin &
L'activité reste dans la fenêtre autorisée. \\

\codecell{required\_\allowbreak resource} &
\codecell{assignment[a,r] = true} &
La ressource requise participe à l'activité. \\

\codecell{preferences} &
variable \codecell{penalty} &
Les violations souples sont évaluées par le même score. \\
\end{longtable}
\endgroup
